...
public class PostDAO {
	...
	public Vector<PostVO> selectAddress(String dong) throws SQLException{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Vector<PostVO> list = new Vector<PostVO>();//리턴할 ArrayList 객체 생성
		try {
			...
			while(rs.next()){
				...
				list.addElement(new PostVO(postNo, sido, gugun, dong, bunji, zipcode, ri, bldg));//리스트에 주소 추가
			}
	...
	//시퀀스로 주소를 찾는 메소드
	public PostVO selectAddress(int post) throws SQLException{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			pstmt.setInt(1, post);//입력받은 dong값 세팅
			...
			while(rs.next()){
				...
				String ri = rs.getString(7);
				String bldg = rs.getString(8);
				return new PostVO(postNo, sido, gugun, dong, bunji, zipcode, ri, bldg);//리스트에 주소 추가
			}
